...
namespace XMath
{
	// 常量定义
	/////////////////////////////////////////////////////////////////////////////////
	float const _PI		= 3.141592f;			// PI
	float const PI2		= 6.283185f;			// PI * 2
	float const HALFPI	= 1.570796f;			// PI / 2
	float const _1divPI = 0.318310f;			// 1 / PI
    ...
    float const XZERO = 0.0000001f;             // 0
	float const BigNumber = 1e37f;              // 很大的数
	...
	template <typename T>
		inline T Clamp(T const & val, T const & low, T const & high)
	{
		T r = val > low ? val : low;
	...
	{
		return DotHelper<typename T::value_type, T::elem_num>::Do(&lhs[0], &rhs[0]);
	}
	...
	template <typename T>
		inline T
		Lerp(T const & lhs, T const & rhs, float t)
	{
